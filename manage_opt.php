	...
	if (isset($_POST['voting_id']) && isset($_POST['opt_txt'])) {
		...
		$partylist_id = $_POST['partylist_id']; // Add this line to get the partylist_id

		$stmt = $conn->prepare("INSERT INTO voting_opt (voting_id, opt_txt, partylist_id) VALUES (?, ?, ?)");
		$stmt->bind_param("iss", $voting_id, $opt_txt, $partylist_id);
		$stmt->execute();
		$stmt->close();
	} else {
	}
	...
	<div class="container-fluid">
		<div class="col-lg-12">
			<form action="" id="manage-opt">
				...
				<div class="form-group">
					<label for="" class="control-label">Party list</label>
					<select name="partylist_id" id="partylist" class="custom-select browser-default">
						<?php
						$cats = $conn->query("SELECT * FROM party_list order by id asc");
						while ($row = $cats->fetch_assoc()) :
						?>
							<option value="<?php echo $row['id'] ?>" <?php echo isset($partylist_id) && $partylist_id == $row['id'] ? 'selected' : '' ?>>
								<?php echo $row['partylist'] ?>
							</option>
						<?php endwhile; ?>
					</select>
				</div>
